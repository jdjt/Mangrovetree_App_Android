 ...
 */
@InLayer(value = R.layout.register)
public class RegisterPhoneFragment extends FragmentActivity {
...
//        register_email_btn.setText("无法获得手机验证码?");
        ...
        //同意协议
        read_agreement.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                //如果是选择状态
                if(read_agreement.isChecked()){
                    register_button.setEnabled(false);
                    register_button.setBackgroundResource(R.color.gray_3);
                }else {
                    register_button.setEnabled(true);
                    register_button.setBackgroundResource(R.drawable.text_state_whitetogreen);
                }
            }
        });
    ...
    private void click(View view){
        switch (view.getId()) {
            ...
            case R.id.register_button:
                //验证
                validator = new Validator(getApplication());
...
//            发送验证码
            case R.id.register_valitation:
                ...
                if (!account.matches(Rules.REGEX_TELPHONE)) {
                    CommonUtils.onErrorToast(register_account, "请输入正确的手机号", this);
                    return;
                }
                //验证手机是否联网
                ...
                if (MapVo.get("register_tel_valitation") != null) {
                    Toast.makeText(this, "验证点击过于频繁,请一分钟后再重新验证", Toast.LENGTH_SHORT).show();
                    return;
                } else {
                    uuid = Uuid.getUuid();//用于参数的uuid
                    MapVo.set("register_tel_valitation_uuid", uuid);
                }
                ((LonginAndRegisterFragmentActivity) this).showDialog();
                inConfig.setKey(4);
                //验证账号是否已注册
    ...
    private void startFragmentAdd(RegisterPhoneFragment registerPhoneFragment) {
...
//    }
     ...
     */
//    @InjectMethod(@InjectListener(ids = {register_button, R.id.register_valitation,  R.id.register_agree_text}, listeners = {OnClick.class}))
